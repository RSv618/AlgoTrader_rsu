...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    stdev: pl.Expr = price.rolling_std(lookback) * threshold
    uptrend_trigger: pl.Expr = (price < price.shift(1) - stdev.shift(1))
    downtrend_trigger: pl.Expr = (price > price.shift(1) + stdev.shift(1))
    
    df = df.with_columns(
        stdev=c.rolling_std(parameter['stdev']).cast(pl.Float64),
        stdev_2=stdev.cast(pl.Float64),
...
def parameters(routine: str | None = None) -> list:
    ...
    match routine:
        case 'parameter_range':
            ...
            threshold: list[float] = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0]
        case _:
            stdev = [512]
