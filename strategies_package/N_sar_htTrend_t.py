...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    h: pl.Expr = plta.ht_trendline(pl.col('high'))
    l: pl.Expr = plta.ht_trendline(pl.col('low'))

    sar: pl.Expr = plta.sar(high=h, low=l,
                            acceleration=parameter['acceleration'], maximum=parameter['threshold'])
    uptrend_trigger_init: pl.Expr = sar <= l
    downtrend_trigger_init: pl.Expr = sar >= h
...
def parameters(routine: str | None = None) -> list:
    ...
    headers: list[str] = ['stdev', 'acceleration', 'threshold']
    match routine:
        case 'parameter_range':
            stdev: list[int] = [8, 16, 32, 64, 128, 256, 512]
            acceleration: list[float] = [0.01, 0.02, 0.04]
            threshold: list[float] = [0.08, 0.16, 0.32, 0.64]
        case _:
            stdev = [512]
            acceleration = [0.01]
            threshold = [0.16, 0.32, 0.64]
