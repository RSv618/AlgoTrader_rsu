...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    atr: pl.Expr = plta.atr(timeperiod=lookback) * threshold
    uptrend_trigger: pl.Expr = (c > c.shift(1) + atr.shift(1))
    downtrend_trigger: pl.Expr = (c < c.shift(1) - atr.shift(1))
    
    df = df.with_columns(
        stdev=c.rolling_std(parameter['stdev']).cast(pl.Float64),
        atr=atr.cast(pl.Float64),
