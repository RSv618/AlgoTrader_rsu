...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    uptrend_trigger: pl.Expr = (bull_count < bear_count) & (bear_count >= threshold)
    downtrend_trigger: pl.Expr = (bull_count > bear_count) & (bull_count >= threshold)

    df = df.with_columns(
        stdev=c.rolling_std(parameter['stdev']).cast(pl.Float64),
        bull_count=bull_count.cast(pl.Int32),
