...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    scaled_stdev: pl.Expr = c.rolling_std(lookback) * threshold
    average_close: pl.Expr = c.rolling_mean(lookback)
    upper: pl.Expr = average_close + scaled_stdev
    lower: pl.Expr = average_close - scaled_stdev

    uptrend_trigger_init: pl.Expr = (c > lower)
    downtrend_trigger_init: pl.Expr = (c < upper)
    ...
    df = df.with_columns(
        ...
        average_close=average_close.cast(pl.Float64),
...
def parameters(routine: str | None = None) -> list:
    ...
    match routine:
        case 'parameter_range':
            ...
            threshold: list[float] = [0.5, 1.0, 1.5, 2.0]
        case _:
            stdev = [512]
            lookback = [64, 128, 512]
            threshold = [2]
