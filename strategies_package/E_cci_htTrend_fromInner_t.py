...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    cci: pl.Expr = plta.ht_trendline(plta.cci(timeperiod=lookback))
    uptrend_trigger_init: pl.Expr = (cci > threshold)
    downtrend_trigger_init: pl.Expr = (cci < -threshold)
...
def parameters(routine: str | None = None) -> list:
    ...
    match routine:
        case 'parameter_range':
            ...
            threshold: list[int] = [50, 100, 150]
        case _:
            stdev = [512]
            lookback = [8, 128, 256, 512]
            threshold = [100, 150]
