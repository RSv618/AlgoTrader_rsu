...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    scaled_atr: pl.Expr = plta.atr(timeperiod=lookback) * threshold
    average_close: pl.Expr = c.rolling_mean(lookback)
    upper: pl.Expr = average_close + scaled_atr
    lower: pl.Expr = average_close - scaled_atr

    uptrend_trigger_init: pl.Expr = (c > upper)
    downtrend_trigger_init: pl.Expr = (c < lower)
