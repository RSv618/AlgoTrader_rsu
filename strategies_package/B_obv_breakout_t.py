...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    obv: pl.Expr = plta.obv()
    entry_high: pl.Expr = obv.rolling_max(lookback)
    entry_low: pl.Expr = obv.rolling_min(lookback)
    uptrend_trigger_init: pl.Expr = (obv > entry_high.shift(1))
    downtrend_trigger_init: pl.Expr = (obv < entry_low.shift(1))
