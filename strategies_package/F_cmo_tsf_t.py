...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    cmo: pl.Expr = plta.cmo(timeperiod=lookback)
    tsf: pl.Expr = plta.tsf(cmo, timeperiod=lookback)
    uptrend_trigger_init: pl.Expr = (cmo > tsf)
    downtrend_trigger_init: pl.Expr = (cmo < tsf)
