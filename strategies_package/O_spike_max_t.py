...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    stdev: pl.Expr = c.rolling_std(lookback)
    spike_diff: pl.Expr = (c - c.shift(1)) / stdev
    spike_magnitude: pl.Expr = np.abs(spike_diff)
    uptrend_trigger: pl.Expr = (spike_magnitude > spike_magnitude.rolling_max(lookback).shift(1)) & (spike_diff > 0)
    downtrend_trigger: pl.Expr = (spike_magnitude > spike_magnitude.rolling_max(lookback).shift(1)) & (spike_diff < 0)

    df = df.with_columns(
        stdev=c.rolling_std(parameter['stdev']).cast(pl.Float64),
        spike_diff=spike_diff.cast(pl.Float64),
