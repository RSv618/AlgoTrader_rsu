...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    signal: int = parameter['signal']

    c: pl.Expr = pl.col('close')

    price: pl.Expr = plta.wclprice()
    macd: pl.Expr = plta.macd(price, fast, slow, signal)
    macd_df: pl.DataFrame = df.select(macd=macd).unnest('macd')
    macd_histogram: Any = macd_df['macdhist']
    uptrend_trigger_init: pl.Expr = macd_histogram > 0
    downtrend_trigger_init: pl.Expr = macd_histogram < 0
    ...
    df = df.with_columns(
        ...
        macd_histogram=macd_histogram.cast(pl.Float64),
...
def parameters(routine: str | None = None) -> list:
    ...
    headers: list[str] = ['stdev', 'slow', 'fast_m', 'signal']
    match routine:
    ...
    values: Any = iter_product(stdev, slow, fast_m, signal)
