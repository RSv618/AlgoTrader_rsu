...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    trix: pl.Expr = plta.trix(c, lookback)
    entry_high: pl.Expr = trix.rolling_max(lookback)
    entry_low: pl.Expr = trix.rolling_min(lookback)
    uptrend_trigger_init: pl.Expr = trix < entry_low.shift(1)
    downtrend_trigger_init: pl.Expr = trix > entry_high.shift(1)
