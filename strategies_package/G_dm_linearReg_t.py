...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    plus_dm: pl.Expr = plta.linearreg_angle(plta.plus_dm(timeperiod=lookback), timeperiod=lookback)
    minus_dm: pl.Expr = plta.linearreg_angle(plta.minus_dm(timeperiod=lookback), timeperiod=lookback)
    uptrend_trigger_init: pl.Expr = (plus_dm > 0) & (minus_dm < 0)
    downtrend_trigger_init: pl.Expr = (plus_dm < 0) & (minus_dm > 0)
    ...
    df = df.with_columns(
        ...
        linear_regression_angle=plus_dm.cast(pl.Float64),
