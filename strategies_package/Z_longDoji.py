...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    pattern: pl.Expr = plta.cdllongleggeddoji() > 0
    uptrend_trigger: pl.Expr = pattern & (c > c.rolling_mean(lookback))
    downtrend_trigger: pl.Expr = pattern & (c < c.rolling_mean(lookback))

    # df_pd = df_pd.with_columns(pattern=pattern, max=pattern.max(), min=pattern.min())
    df = df.with_columns(
...
def parameters(routine: str | None = None) -> list:
    ...
    headers: list[str] = ['stdev', 'lookback', 'count']
    match routine:
    ...
    values: Any = iter_product(stdev, lookback, count)
