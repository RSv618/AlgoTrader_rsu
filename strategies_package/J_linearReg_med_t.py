...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    linear_angle: pl.Expr = plta.linearreg_angle(price, lookback)
    uptrend_trigger_init: pl.Expr = linear_angle > threshold
    downtrend_trigger_init: pl.Expr = linear_angle < -threshold
    ...
    df = df.with_columns(
        ...
        linear_regression_angle=linear_angle.cast(pl.Float64),
