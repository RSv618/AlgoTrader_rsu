...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    ht_trend: pl.Expr = plta.ht_trendline(c)
    entry_high: pl.Expr = ht_trend.rolling_max(lookback)
    entry_low: pl.Expr = ht_trend.rolling_min(lookback)
    exit_high: pl.Expr = ht_trend.rolling_max(exit_lookback)
    exit_low: pl.Expr = ht_trend.rolling_min(exit_lookback)

    uptrend_trigger_init: pl.Expr = (ht_trend > entry_high.shift(1))
    uptrend_exit_trigger_init: pl.Expr = (ht_trend < exit_low.shift(1))
    downtrend_trigger_init: pl.Expr = (ht_trend < entry_low.shift(1))
    downtrend_exit_trigger_init: pl.Expr = (ht_trend > exit_high.shift(1))
