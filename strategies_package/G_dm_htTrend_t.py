...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    plus_dm: pl.Expr = plta.ht_trendline(plta.plus_dm(timeperiod=lookback))
    minus_dm: pl.Expr = plta.ht_trendline(plta.minus_dm(timeperiod=lookback))
    ...
    df = df.with_columns(
        ...
        plus_dm=plus_dm.cast(pl.Float64),
