...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    bop: pl.Expr = plta.bop()
    pos_count: pl.Expr = bop.ge(0).cast(pl.Int32).rolling_sum(window_size=lookback)
    neg_count: pl.Expr = bop.le(0).cast(pl.Int32).rolling_sum(window_size=lookback)
    positive_average: pl.Expr = pl.when(bop >= 0).then(bop).otherwise(None).rolling_mean(window_size=lookback,
                                                                                         min_periods=1)
    positive_average = pl.when(pos_count <= 0).then(0).otherwise(positive_average)
    negative_average: pl.Expr = pl.when(bop <= 0).then(-bop).otherwise(None).rolling_mean(window_size=lookback,
                                                                                          min_periods=1)
    negative_average = pl.when(neg_count <= 0).then(0).otherwise(negative_average)
    uptrend_trigger_init: pl.Expr = (positive_average > negative_average)
    downtrend_trigger_init: pl.Expr = (positive_average < negative_average)
    ...
    df = df.with_columns(
        ...
        pos_count=pos_count.cast(pl.Int32),
