...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    plus_dm: pl.Expr = plta.tsf(plta.plus_dm(timeperiod=lookback), timeperiod=lookback)
    minus_dm: pl.Expr = plta.tsf(plta.minus_dm(timeperiod=lookback), timeperiod=lookback)
    uptrend_trigger_init: pl.Expr = (plus_dm < minus_dm)
    downtrend_trigger_init: pl.Expr = (plus_dm > minus_dm)
