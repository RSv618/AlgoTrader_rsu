...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    macd_series: pl.Series = macd_df['macd']
    entry_high: pl.Expr = macd_series.rolling_max(fast)
    entry_low: pl.Expr = macd_series.rolling_min(fast)
    uptrend_trigger_init: pl.Expr = macd_series < entry_low.shift(1)
    downtrend_trigger_init: pl.Expr = macd_series > entry_high.shift(1)
