...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    fastd: pl.Expr = pl.col('fastd')
    uptrend_trigger: pl.Expr = (fastk >= threshold) & (fastk > fastd) & (fastk.shift(1) <= fastd.shift(1))
    downtrend_trigger: pl.Expr = (fastk <= 100 - threshold) & (fastk < fastd) & (fastk.shift(1) >= fastd.shift(1))
