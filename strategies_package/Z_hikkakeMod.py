...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    pattern: pl.Expr = plta.cdlhikkakemod()
    uptrend_trigger: pl.Expr = pattern == threshold
    downtrend_trigger: pl.Expr = pattern == -threshold
...
def parameters(routine: str | None = None) -> list:
    ...
    headers: list[str] = ['stdev', 'count', 'threshold']
    match routine:
