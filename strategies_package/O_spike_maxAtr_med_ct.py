...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    spike_magnitude: pl.Expr = np.abs((price - price.shift(1))/plta.atr(timeperiod=lookback))
    uptrend_trigger: pl.Expr = ((spike_magnitude > spike_magnitude.rolling_max(lookback).shift(1)) &
                                (price < price.shift(1)))
    downtrend_trigger: pl.Expr = ((spike_magnitude > spike_magnitude.rolling_max(lookback).shift(1)) &
                                  (price > price.shift(1)))
