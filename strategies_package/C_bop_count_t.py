...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    threshold: float = parameter['threshold'] * lookback

    c: pl.Expr = pl.col('close')

    bop: pl.Expr = plta.bop().gt(0).cast(pl.Int32).rolling_sum(window_size=lookback)
    uptrend_trigger_init: pl.Expr = (bop > threshold)
    downtrend_trigger_init: pl.Expr = (bop < threshold)
...
def parameters(routine: str | None = None) -> list:
    ...
    match routine:
        case 'parameter_range':
            ...
            threshold: list[float] = [0.25, 0.5, 0.75]
        case _:
            stdev = [512]
            lookback = [32, 256]
            threshold = [0.25, 0.75]
