...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    ht_trend: pl.Expr = plta.ht_trendline()
    slow_ma: pl.Expr = ht_trend.rolling_mean(lookback)
    uptrend_trigger_init: pl.Expr = ht_trend > slow_ma
    downtrend_trigger_init: pl.Expr = ht_trend < slow_ma
    uptrend_filter: pl.Expr = (plta.ht_trendmode() == pl.lit(1))
    ...
    df = df.with_columns(
        ...
        slow_ma=slow_ma.cast(pl.Float64),
