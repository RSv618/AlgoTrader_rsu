...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    slow_ma: pl.Expr = plta.ma(c, lookback)
    diff: pl.Expr = c - slow_ma
    diff_high: pl.Expr = diff.rolling_max(lookback)
    diff_low: pl.Expr = diff.rolling_min(lookback)
    uptrend_trigger_init: pl.Expr = (diff > diff_high.shift(1))
    downtrend_trigger_init: pl.Expr = (diff < diff_low.shift(1))
    ...
    df = df.with_columns(
        ...
        diff_high=diff_high.cast(pl.Float64),
