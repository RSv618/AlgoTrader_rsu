...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    willr: pl.Expr = plta.willr(timeperiod=lookback) + 100.0
    slow_ma: pl.Expr = willr.rolling_mean(lookback)
    uptrend_trigger_init: pl.Expr = willr > slow_ma
    downtrend_trigger_init: pl.Expr = willr < slow_ma
