...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    c: pl.Expr = plta.wclprice()

    price: pl.Expr = plta.wclprice()
    entry_high: pl.Expr = price.rolling_max(lookback)
    entry_low: pl.Expr = price.rolling_min(lookback)
    exit_high: pl.Expr = price.rolling_max(exit_lookback)
    exit_low: pl.Expr = price.rolling_min(exit_lookback)

    uptrend_trigger_init: pl.Expr = (price > entry_high.shift(1))
    uptrend_exit_trigger_init: pl.Expr = (price < exit_low.shift(1))
    downtrend_trigger_init: pl.Expr = (price < entry_low.shift(1))
    downtrend_exit_trigger_init: pl.Expr = (price > exit_high.shift(1))
