...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    df = df.with_columns(macd=macd_series)
    tsf: pl.Expr = plta.tsf(pl.col('macd'), timeperiod=fast)
    uptrend_trigger_init: pl.Expr = pl.col('macd') > tsf
    downtrend_trigger_init: pl.Expr = pl.col('macd') < tsf
