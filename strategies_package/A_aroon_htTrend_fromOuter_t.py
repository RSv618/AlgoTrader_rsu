...
def indicators(df: pl.DataFrame, parameter: dict[str, Any]) -> pl.DataFrame:
    ...
    aroon: pl.Expr = plta.ht_trendline(plta.aroonosc(timeperiod=lookback))
    uptrend_trigger_init: pl.Expr = (aroon > -threshold)
    downtrend_trigger_init: pl.Expr = (aroon < threshold)
...
def parameters(routine: str | None = None) -> list:
    ...
    match routine:
        case 'parameter_range':
            ...
            threshold: list[int] = [0, 25, 50, 75]
        case _:
            stdev = [512]
            lookback = [8, 16, 32, 64, 128, 256, 512]
            threshold = [0, 25, 50, 75]
